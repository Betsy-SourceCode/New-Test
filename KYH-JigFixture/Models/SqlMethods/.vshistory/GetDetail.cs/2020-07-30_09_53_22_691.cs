...
namespace Customer.Models.SqlMethods
{
    public class GetDetail
    {
        ...
        {
            Others list = db.Database.SqlQuery<Others>(@"SELECT COUNT(1) AS id FROM dbo.Customer_AuditInfo c  where c.CustomerId=@CustomerId", new SqlParameter("@CustomerId", CustomerId)).FirstOrDefault();
            return list;
        }
    }
}