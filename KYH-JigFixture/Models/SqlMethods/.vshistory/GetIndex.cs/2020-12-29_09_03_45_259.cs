...
namespace Customer.Models.SqlMethods
{
    public class GetIndex
    {
        ...
        {
            //模糊查询
            string queryIndexSql = "select * from SelectKnowledgeBaseIndex q where (isnull(q.Question,'') like '%{0}%' or isnull(q.KeyWord,'') like '%{0}%' or isnull(q.Answer,'') like '%{0}%') and q.TopicArea not in('{1}')";
