...
namespace KYH_JigFixture.Models.SqlMethods
{
    public class GetIndex
    {
        ...
        /// <returns></returns>
        public List<SelectOA_Result> GetIndexPageRowOA(string OAFlow_Num, string Start_Date, string End_Date, string Mat_Code, string Mat_Name)
        {
            //默认显示非结案
            //带过滤条件查询OA数据
            string queryIndexSql = "exec SelectOA '{0}','{1}','{2}','{3}','{4}'";
            queryIndexSql = string.Format(queryIndexSql, OAFlow_Num, Start_Date, End_Date, Mat_Code, Mat_Name);
