...
namespace Customer.Models.SqlMethods
{
    public class GetIndex
    {
        ...
        public List<SelectOA_Result> ByK3PO_NumSelect(string)
        {
            string queryIndexSql = @" SELECT POOrder.FBillNo,POOrder.FDate,t_Supplier.FName as Supplier_FName,
 t_item.FName as item_FName,t_measureunit.FName as k3Unit,POOrderEntry.FQty as K3FQty
  FROM HS_YHNPN.dbo.POOrder
 INNER JOIN HS_YHNPN.dbo.POOrderEntry ON POOrderEntry.FInterID = POOrder.FInterID
 INNER JOIN HS_YHNPN.dbo.t_Supplier ON t_Supplier.FItemID = POOrder.FSupplyID
 INNER JOIN HS_YHNPN.dbo.t_Item ON t_Item.FItemID=POOrder.FEmpID
 INNER JOIN HS_YHNPN.dbo.t_icitem ON t_ICItem.FItemID = POOrderEntry.FItemID
 INNER JOIN HS_YHNPN.dbo.t_measureunit ON POOrderEntry.FUnitID = t_measureunit.FItemID";
