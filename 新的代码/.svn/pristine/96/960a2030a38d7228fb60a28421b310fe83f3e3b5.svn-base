...
namespace KYH_NewEmailDomainPT.Models
{
    public class GetIndex
    {
        ...
        /// <returns></returns>
        public List<NewEmailDomain2GIP_Select> GetIndexListSql(NewEmailDomain2GIP_Select list, string EndTime)
        {
            string sql = @"select * from [dbo].[NewEmailDomain2GIP_Select]
            where DomainName like '%{0}%' and FindDate>= '{1}' and CONVERT(NVARCHAR(10),FindDate,23)<= '{2}' order by findDate desc";
