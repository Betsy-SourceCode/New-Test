...
namespace KYH_JigFixture.Models.SqlMethods
{
    public class GetIndex
    {
        ...
        public List<Others> ByK3PO_NumSelect(string Bname, string FBillNo)
        {
            try
            {
                ...
                //  Others totalCount = Webdb.Database.SqlQuery<Others>(queryIndexSql).ToList();
                List<Others> totalCount = c6db.Database.SqlQuery<Others>(queryIndexSql).ToList();
                return totalCount;
            }
            catch (Exception ex)
            {
