...
namespace KYH_NewEmailDomainPT.Controllers
{
    public class NewEmailDomainPTController : Controller
    {
        ...
        /// <returns></returns>
        public string IndexData(NewEmailDomain2GIP_Select NewEmailDomain, string TopicArea, string EndTime, string orderby, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                List<NewEmailDomain2GIP_Select> data = new GetIndex().GetIndexListSql(NewEmailDomain, EndTime, orderby);
