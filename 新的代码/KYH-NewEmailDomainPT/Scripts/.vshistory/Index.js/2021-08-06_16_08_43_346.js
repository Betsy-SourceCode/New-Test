...
app.controller('mycontroller', function ($scope) {
                ...
                if (list.length == 0) {
                    ...
                    $("#myPage").sPage({
                        ...
                        backFun: function (page) {
                            $scope.IndexList(page, 10); //点击分页按钮回调函数，返回当前页码
                        }
    ...
    //修改新到的电邮域名信息模态框
    $scope.OpenModal = function (DomainName, FindDate, SourceType, Remarks) {
        //给模态框赋值
        document.getElementById('MTDomainName').innerText = DomainName;
        document.getElementById('MTFindDate').innerText = FindDate;
        document.getElementById('MTSourceType').innerText = SourceType;
        document.getElementById('MTRemarks').innerText = Remarks;
    }
    //修改新到的电邮域名信息方法
    $scope.Update = function () {
        ...
        var SourceType = document.getElementById('MTSourceType').innerText;
        var Remarks = document.getElementById('MTRemarks').innerText;
    }
    $scope.IndexList(1);
    //swal('Good job!', 'You clicked the button!', 'success') //提示框
})
//格式化时间
